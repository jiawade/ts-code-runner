...
import {configFilePath, readConfigFile} from "./form";
...
export async function buildCommand() {
    ...
    const parameters = config.nodeParameters !== '' ? config.nodeParameters : '';
    const script = path.join(config.workingDirectory, config.javascriptFile);
    return `${config.nodeInterpreter} ${parameters} ${script} ${config.applicationParameter}`;
}
...
export async function Run() {
    ...
    const executor = await buildCommand()

    await ExecuteCommandInOutputChannel(executor);
}
...
export async function DebugRun() {
    ...
    await ExecuteCommandInTerminal(executor);
}
...
export function showConfigQuickPick() {
    ...
    quickPick.onDidAccept(() => {
        ...
        quickPick.dispose();
        vscode.window.showInformationMessage("配置已应用").then();
    });
